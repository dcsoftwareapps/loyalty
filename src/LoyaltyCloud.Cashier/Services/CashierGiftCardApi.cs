...
public sealed class CashierGiftCardApi(AuthenticatedCashierApiClient api) : ICashierGiftCardApi
{
    ...
    private async Task<GiftCardResult<T>> SendAsync<T, TRequest>(string path, TRequest request)
    {
        try
        {
            ...
            {
                (400, "InvalidInput") => GiftCardResult<T>.Fail("InvalidInput", "Revisa el código y el monto (máximo dos decimales).", true),
                ...
                (409, "IdempotencyConflict") => GiftCardResult<T>.Fail("IdempotencyConflict", "Este canje requiere revisión: la clave corresponde a otra operación. Contacta al administrador."),
                _ => Unknown<T>()
            };
        }
