...
internal sealed class BillingService : IBillingService
{
    ...
    private readonly IMutableTenantContext _tenantContext; private readonly ILogger<BillingService> _logger; private readonly IDataProtector _returnProtector; private readonly IBillingNotificationService _notifications;
    public BillingService(AppDbContext db, IDateTimeProvider clock, IPaymentGateway gateway, IMutableTenantContext tenantContext, ILogger<BillingService> logger, IDataProtectionProvider dataProtection, IBillingNotificationService notifications)
    { _db=db; _clock=clock; _gateway=gateway; _tenantContext=tenantContext; _logger=logger; _returnProtector=dataProtection.CreateProtector("LoyaltyCloud.Billing.Return.v1"); _notifications=notifications; }
    ...
    { var t=await RequireTenant(tenantId,ct); _tenantContext.SetTenant(t.Id,t.Slug); var p=await Profile(tenantId,ct); if(p.StripeSubscriptionId is not null) await _gateway.SetSubscriptionCancellationAsync(p.StripeSubscriptionId,!enabled,ct); p.SetAutoRenew(enabled); if(billingContactEmail is not null)p.SetContactEmail(billingContactEmail); await _db.SaveChangesAsync(ct); }
    ...
    =>_notifications.SendAsync(new BillingNotification(t.Id,p.BillingContactEmail,type,c.InvoiceId??c.EventId,c.AmountTotalMinor/100m,c.Currency,effective,grace,$"/{t.Slug}/billing"),ct);    private async Task ReconcileExpiredCardOrdersAsync(Guid tenantId,CancellationToken ct)
    {
