using KBeauty.Loyalty.Application.Customers.Commands.JoinCustomer;
...
public sealed class PublicJoinController : ControllerBase
{
    ...
    public async Task<IActionResult> Join([FromBody] PublicJoinRequest body, CancellationToken ct)
    {
        ...
        if (result.IsFailure)
            return BadRequest(new ProblemDetails { Title = "Alta KBeauty Loyalty", Detail = result.Error });
