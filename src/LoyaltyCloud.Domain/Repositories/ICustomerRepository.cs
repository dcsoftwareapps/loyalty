...
{
    /// <summary>Busca un cliente por Id. Devuelve <c>null</c> si no existe.</summary>
    ...
    /// <summary>Busca el cliente a través del serial de su <see cref="LoyaltyCard"/>.</summary>
    Task<Customer?> GetBySerialNumberAsync(string serialNumber, CancellationToken ct = default);

    /// <summary>Indica si existe ya un cliente con ese email (sin traer la entidad).</summary>
