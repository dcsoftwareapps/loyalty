...
public sealed record IssueGiftCardRequest(decimal Amount, Guid? RecipientMemberId, string RecipientName, string? RecipientEmail, string? RecipientPhone, string? SenderName, string? PersonalMessage, GiftCardSource Source = GiftCardSource.Manual, DateTime? ExpiresAtUtc = null, string? IdempotencyKey = null);
